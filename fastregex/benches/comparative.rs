...
fn bench_comparatively(c: &mut Criterion){
    ...
    for haystack in haystack {
        ...
        group.bench_with_input(BenchmarkId::new("Fastregex", haystack), haystack, |b, i| b.iter(|| https_matcher(black_box(i))));
        group.bench_with_input(BenchmarkId::new("Traditional Regex", haystack), haystack, |b, i| b.iter(|| regex.is_match(black_box(i))));
        group.finish();
    }
}
